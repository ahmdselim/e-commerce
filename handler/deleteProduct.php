...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    if (isset($_GET["id"])) {
        ...
        if (isset($data)) {
            $selectItems = "DELETE FROM `items` WHERE id='$id'";
            mysqli_query($con, $selectItems);
            $image =  $data[0]['image'];
            unlink("../uploads/products/$image");

            $_SESSION["success"] = "congrats :) your product delete successfully keep going 👍";
            header("location:../pages/dashboardProducts.php");
        } else {
            echo <<<TYPE
